...
def prepareMJplots(returns, positions=None,
                   transactions=None,
    ...
    ax_returns = pf.plotting.plot_returns(
        returns,
        live_start_date=live_start_date)
    ...
    ax_rolling_returns = pf.plotting.plot_rolling_returns(
        returns,
    ...
    ax_rolling_returns_vol_match = pf.plotting.plot_rolling_returns(
        returns,
        factor_returns=benchmark_rets,
        live_start_date=live_start_date,
        cone_std=None,
    ...
    ax_rolling_returns_log = pf.plotting.plot_rolling_returns(
        returns,
        factor_returns=benchmark_rets,
        logy=True,
        live_start_date=live_start_date,
    ...
    print("Plotting beta can be slow!")
    ax_rolling_beta = pf.plotting.plot_rolling_beta(
        returns, benchmark_rets)
    plt.show()

    ax_rolling_volatility = pf.plotting.plot_rolling_volatility(
        returns, factor_returns=benchmark_rets)
    plt.show()

    ax_rolling_sharpe = pf.plotting.plot_rolling_sharpe(
        returns)
    plt.show()

    print("Fama french can be slow!")
    ax_rolling_risk = pf.plotting.plot_rolling_fama_french(
        returns)
    plt.show()

    # Drawdowns
    ax_drawdown = pf.plotting.plot_drawdown_periods(
        returns, top=5)
    plt.show()

    ax_underwater = pf.plotting.plot_drawdown_underwater(
        returns=returns)
    plt.show()

    ax_monthly_heatmap = pf.plotting.plot_monthly_returns_heatmap(returns)
    plt.show()

    ax_annual_returns = pf.plotting.plot_annual_returns(returns)
    plt.show()

    ax_monthly_dist = pf.plotting.plot_monthly_returns_dist(returns)
    plt.show()

    print("Plotting return quantiles can be slow...")
    ax_return_quantiles = pf.plotting.plot_return_quantiles(
        returns,
        live_start_date=live_start_date)
    plt.show()

    if bootstrap:
        print("This can be VERY slow...")
        ax_bootstrap = pf.plotting.plot_perf_stats(returns, benchmark_rets)
        plt.show()
...
if False:
    ...
    prepareMJplots(stock_rets, live_start_date='2015-12-1')
